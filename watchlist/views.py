...
def add_to_watchlist(request, movie_id):
    ...
    if not movie:
        tmdb_movie_url = f"https://api.themoviedb.org/3/movie/{movie_id}?api_key={
    ...
    watchlist, is_created = Watchlist.objects.get_or_create(
...
def search_movies(request):
    ...
    tmdb_search_url = f"https://api.themoviedb.org/3/search/movie?api_key={
        settings.TMDB_API_KEY}&query={query}"
...
def view_watchlist(request):
    ...
    for entry in watchlist:
        tmdb_movie_url = f"https://api.themoviedb.org/3/movie/{
            entry.movie.tmdb_id}?api_key={
            settings.TMDB_API_KEY}"
        response = requests.get(tmdb_movie_url)

        if response.status_code == 200:
            data = response.json()
            entry.movie.poster = f"https://image.tmdb.org/t/p/w500{
                data['poster_path']}"
